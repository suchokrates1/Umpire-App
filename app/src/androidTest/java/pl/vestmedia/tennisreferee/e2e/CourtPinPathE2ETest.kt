...
/**
 * Court PIN → PlayerSelection path (not a direct MatchActivity launch).
 *
 * Args: `e2e.baseUrl`, `e2e.adminPassword` (same as MultiCourt).
 */
@RunWith(AndroidJUnit4::class)
@LargeTest
class CourtPinPathE2ETest {
    ...
    @Test
    fun courtPin_opensPlayerSelection() {
        val intent = Intent(
        ...
        ActivityScenario.launch<CourtSelectionActivity>(intent).use { scenario ->
            ...
            onView(withId(R.id.recyclerViewPlayers)).check(matches(isDisplayed()))
            assertTrue("Player selection reached after PIN", true)
        }
    }
